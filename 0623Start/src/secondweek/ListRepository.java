...
public class ListRepository{
	...
	public List<EnrollParticipate> getEnrollment(int userno) {
		...
		try {
			...
			String sql2 = "SELECT B.setno, A.userno, C.productno, C.productname, C.image, D.deadline, D.finalprice FROM user A\n" + 
					"INNER JOIN enrollmentinfo B ON A.userno = B.userno\n" + 
					"INNER JOIN product C ON B.productno = C.productno\n" + 
					"INNER JOIN copy_auction D ON B.setno = D.setno\n" + 
					"WHERE A.userno = ?\n" + 
	...
	public List<EnrollParticipate> getParticipateList(int userno) {
		...
		try {
			...
			String sql2 = "SELECT C.setno, A.userno, E.productno, E.productname, E.image, C.finalprice, C.deadline\n" + 
					"FROM user A\n" + 
					"INNER JOIN (\n" + 
					"    SELECT p.*\n" + 
					"    FROM participate p\n" + 
